...
class DBManager:
    ...
    def get_students_with_attendance(self) -> List[Tuple]:
        ...
        self.cursor.execute("""
            SELECT s.student_id, s.name, s.course, s.email,
    ...
    def get_all_grades(self) -> List[Tuple]:
        ...
        self.cursor.execute("""
            SELECT g.student_id, s.name, g.assessment_type, g.assessment_name, 
                   g.score, g.max_score, g.date
            FROM grades g
    ...
    def reset_grading_config(self):
        ...
        for component, weight in default_config:
            self.cursor.execute(
                "UPDATE grading_config SET weight = ? WHERE component = ?",
                (weight, component)
            )
